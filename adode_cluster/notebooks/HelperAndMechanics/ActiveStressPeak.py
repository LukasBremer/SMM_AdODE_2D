...
def index_finder(maxima,minima,dA,start_indx=0):
    ...
    while control:
        ...
        #         break
        #         control = True
        if dA[maxima[max_indx]] < dA[maxima[max_indx+1]] and abs(maxima[max_indx] - maxima[max_indx+1]) < 300:
            'choose next maxima if there is a close neighboured hogher max'
            max_indx += 1
            control = True

        if abs(dA[maxima[max_indx]]/dA.max() - dA[minima[min_indx]]/dA.max()) < 0.2:
